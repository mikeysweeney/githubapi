class PopupClass {
    constructor(elm, timeout, onClickFunction) {
        ...
        this.n = 0;
        this.o = 0;
        if (!timeout) this.t = false;
        else if(typeof timeout === "number" || typeof timeout === "boolean") this.t = timeout;
        else this.t = 5000;
    ...
    show(text, name){
        ...
        if (this.t) setTimeout(function () { _this._closeMessageButton(id); }, this.t);
    }
    hideAll(){
        if(this.a.length>0){
            let i;
            for (i = 0; i < this.a.length; i++) this._closeMessageButton(this.a[i].id);
        }
    }
                    ...
                    if (id === _this.a[i].id) {
                        if (i === _this.a.length - 1) {
                            ...
                            for (a = 0; a < _this.a.length; a++) {
                                e = document.getElementById(_this.a[a].id);
                                h = parseInt(e.style.marginBottom.replace('px', '')) - tmpH - 6;
                                if (a === _this.a.length - 1) h = parseInt(e.style.marginBottom.replace('px', ''));
                                if (_this.a[a + 1] && _this.a[a + 1].id === _this.a[i + 1].id) break;
                                e.style.marginBottom = h + "px";
                            }
                        }
                        _this.a.splice(i, 1);
    ...
    _swipe(_this, event){
        if (event.target.id!=="PM_close_BTN"){
            let mousePosX = event.clientX || Math.round(event.touches[0].clientX),
                ...
                eID = event.target.id;
            if (~event.target.parentNode.id.indexOf("PM_message")) e = event.target.parentNode;
